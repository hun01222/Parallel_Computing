...
void convertAll(Logger& logger) {
    ...
    for(int i=0;i<20;i++) names.push_back("resnet-34_"+std::to_string(i));
    for(auto& n:names) {
...
bool loadCIFAR100(const std::string& f, std::vector<std::vector<float>>& imgs,
    ...
    float mean[3]={0.5071f,0.4865f,0.4409f}, stdv[3]={0.2673f,0.2564f,0.2762f};
    for(int i=0;i<N;i++){
...
int main(){
    // CSV 출력용 파일 열기
    std::ofstream csv("fine_results.csv");
    if(!csv.is_open())
        throw std::runtime_error("Cannot open fine_results.csv for writing");
    // 헤더
    csv << "fine_pred,fine_ans\n";
    ...
    try{
        ...
        for(int i=0;i<20;i++){
            fineE[i]=loadEngine("resnet-34_"+std::to_string(i)+".engine",gLogger);
        ...
        for(size_t i=0;i<images.size();i+=B){
            ...
            for(int k=0;k<b;k++){
                ...
                if(pF == trueF)         corrFi++;
                if(pC==coarseLabels[i+k] && pF==trueF) corrBoth++;
            }
